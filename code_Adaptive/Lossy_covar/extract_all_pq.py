...
def extract_bitstream_data(dataset_name, depth, rest_pq, dc_pq, opacity_pq, thr, mode, suffix, comp_mode, meta_size):
    ...
    covariance_file = os.path.join(
        ...
        f"{dataset_name}_depth_{depth}_covariance.json"
    )
    dc_file = os.path.join(
        bpp_path,
        f"{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossy",
        f"{dataset_name}_depth_{depth}_dc_pq_{dc_pq}.txt"
    )
    opacity_file = os.path.join(
        bpp_path,
        f"{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossy",
    ...
    if os.path.exists(covariance_file):
        with open(covariance_file, 'r', encoding='utf-8') as f:
            ...
            result['covariance_B'] = covariance_data.get('covariance_matrix_bitstream_size', 0)
    else:
    ...
    print(f"Found {len(rest_files)} REST files to process for dataset {dataset_name}, pq={rest_pq}.")
