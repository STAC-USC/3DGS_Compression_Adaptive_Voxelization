...
def render_video(model_path, iteration, views, gaussians, pipeline, background):
    ...
    for idx, pose in enumerate(tqdm(generate_ellipse_path(views, n_frames=600), desc="Rendering progress")):
        ...
        # 调整渲染图像的宽度和高度，使其为偶数
        height, width = rendering.shape[1:3]
        new_height = height if height % 2 == 0 else height + 1
        new_width = width if width % 2 == 0 else width + 1
        rendering = torchvision.transforms.functional.resize(rendering, (new_height, new_width))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_sh_degree", default=3, type=int, help="Maximum spherical harmonics degree")
