...
import gc
from scene.gaussian_model import GaussianModel  # 确保导入 GaussianModel
...
def compress_color(
    ...
    vq_mask_c = ~keep_mask
    print(f"color vq mask: {vq_mask_c.float().mean()*100:.2f}%")
    
    # 打印数据条件
    print(f"color_importance: {color_importance}")
    print(f"keep_mask: {keep_mask}")
    print(f"vq_mask_c: {vq_mask_c}")
    ...
    if vq_mask_c.any():
        ...
        print("Color vector quantization completed.")
    else:
    ...
    print("Color compression and indexing completed.")
...
def compress_covariance(
    ...
    print(f"gaussians vq mask: {vq_mask_g.float().mean()*100:.2f}%")
    
   
    print(f"gaussian_importance: {gaussian_importance}")
    print(f"keep_mask_g: {keep_mask_g}")
    print(f"vq_mask_g: {vq_mask_g}")
    ...
    if vq_mask_g.any():
        ...
        print("Covariance matrix vector quantization completed.")
    else:
    ...
    print("Covariance compression and indexing completed.")
...
def compress_gaussians(
    ...
    with torch.no_grad():
        if color_comp is not None:
            print("Compressing color features with vector quantization...")
            compress_color(
                gaussians,
                color_importance,
                color_comp,
            ...
            print("Skipping color feature vector quantization.")
    
        if gaussian_comp is not None:
            print("Compressing covariance matrices with vector quantization...")
            compress_covariance(
                gaussians,
                gaussian_importance,
                gaussian_comp,
            ...
            print("Skipping covariance matrix vector quantization.")