...
class GaussianModel:
    ...
    def get_scaling(self):
        # Check for NaN values in self._scaling and handle them
        if torch.isnan(self._scaling).any():
            print("Warning: NaN values found in scaling. Replacing NaNs with zeros.")
            self._scaling = torch.nan_to_num(self._scaling, nan=0.0)
    ...
    def save_npz(
        ...
        with torch.no_grad():
            ...
            # Adding logging for byte stream sizes
            total_size = 0
            covariance_size = 0
            ...
            for key, value in save_dict.items():
                ...
                total_size += size
                if key in ["scaling", "scaling_scale", "scaling_zero_point", 
