...
def calc_importance(
    ...
    importance = torch.cat(
        [gaussians._features_dc.grad, gaussians._features_rest.grad], 1
...
def run_vq(
    ...
    print(f"Initial number of Gaussians: {initial_gaussian_count}")
    ...
    start_time = time.time()
    gaussians.save_npz(out_file, model_params.model_path, sort_morton=not comp_params.not_sort_morton)
    end_time = time.time()
    ...
    final_gaussian_count = gaussians.get_xyz.shape[0]
    print(f"Final number of Gaussians: {final_gaussian_count}")
...
if __name__ == "__main__":
    ...
    run_vq(model.extract(args), op.extract(args), pipeline.extract(args), comp.extract(args))
