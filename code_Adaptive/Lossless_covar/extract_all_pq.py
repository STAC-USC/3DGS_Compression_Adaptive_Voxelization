...
def extract_bitstream_data(dataset_name, depth, rest_pq, dc_pq, opacity_pq,thr, mode, suffix,meta_size):
    """Extract bitstream data from bpp folder and compute covariance bitstream size by summing scale and rot sizes."""
    ...
    # Extract scale and rot files and accumulate values
    for i in range(3):  # Iterate over scale_0, scale_1, scale_2
    ...
    for i in range(4):  # Iterate over rot_0, rot_1, rot_2, rot_3   
    ...
    if os.path.exists(dc_file):
        with open(dc_file, 'r', encoding='utf-8') as f:
            for line in f:
                match_pos = re.match(r'positions bitstream size (\d+) B', line.strip())
                match_colors = re.match(r'colors bitstream size (\d+) B', line.strip())
                if match_pos:
...
def merge_psnr_files(psnr_directory, output_filename,thr, mode, suffix,meta_size):
    """Merge JSON files in the PSNR folder and combine with extracted bitstream data."""
    merged_data = {}
    ...
    for filename in os.listdir(psnr_directory):
        if filename.endswith('.json'):
            ...
            if match:
                ...
                # 	Read PSNR data from JSON file
                file_path = os.path.join(psnr_directory, filename)
                with open(file_path, 'r', encoding='utf-8') as f:
                    psnr_data = json.load(f)

                # Extract corresponding bitstream data
                bitstream_data = extract_bitstream_data(
                    dataset_name, depth, rest_pq, dc_pq, opacity_pq,thr, mode, suffix,meta_size
                )

                # Merge PSNR data with bitstream data
...
def main():
    ...
    # Read the size of the meta data file, assumed to be located under base_path/Meta_data
    ...
    merge_psnr_files(psnr_directory, f'PSNR_{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossless.json', thr, mode, suffix,meta_size)
    ...
    merge_psnr_files(psnr_per_view_directory, f'PSNR_per_view_{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossless.json', thr, mode, suffix,meta_size)
