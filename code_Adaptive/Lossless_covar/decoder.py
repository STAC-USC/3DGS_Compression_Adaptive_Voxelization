...
def assign_attributes_and_save(template_ply_file, output_file, denormalized_dc, denormalized_rest, denormalized_opacity, denormalized_scale, denormalized_rot):
    try:
        ...
        # Assign denormalized scale attributes
        for i, prop in enumerate(['scale_0', 'scale_1', 'scale_2']):
            points[prop] = denormalized_scale[i]
            print(f"Assigned {prop}.")

        # Assign denormalized rot attributes
        for i, prop in enumerate(['rot_0', 'rot_1', 'rot_2', 'rot_3']):
            points[prop] = denormalized_rot[i]
            print(f"Assigned {prop}.")
...
def compare_chunk(dc_chunk, other_chunk):
    differences = ~np.isclose(dc_chunk, other_chunk, atol=1e-6)
    return np.any(differences, axis=1)

def compare_points(dc_points, other_points, name):
    ...
    if num_differences == 0:
        ...
        for idx in diff_indices[:5]:
            print(f"Index {idx}: dc_point={dc_points[idx]}, {name}_point={other_points[idx]}")
...
def main():
    ...
    decompress_files(
        os.path.join(output_base_dir, compression_folders["dc"]),
        os.path.join(output_base_dir, decompression_folders["dc"]),
        tmc3_path, os.path.join(output_base_dir, decompression_folders["dc"], ""),
        depth
    )

    decompress_files(
        os.path.join(output_base_dir, compression_folders["opacity"]),
        os.path.join(output_base_dir, decompression_folders["opacity"]),
        tmc3_path, os.path.join(output_base_dir, decompression_folders["opacity"], ""),
        depth
    )

    decompress_files(
        os.path.join(output_base_dir, compression_folders["rest"]),
        os.path.join(output_base_dir, decompression_folders["rest"]),
        tmc3_path, os.path.join(output_base_dir, decompression_folders["rest"], ""),
        depth
    )

    decompress_files(
        os.path.join(output_base_dir, compression_folders["scale"]),
        os.path.join(output_base_dir, decompression_folders["scale"]),
        tmc3_path, os.path.join(output_base_dir, decompression_folders["scale"], ""),
        depth
    )

    decompress_files(
        os.path.join(output_base_dir, compression_folders["rot"]),
        os.path.join(output_base_dir, decompression_folders["rot"]),
        tmc3_path, os.path.join(output_base_dir, decompression_folders["rot"], ""),
        depth
    )

    # Load metadata file for denormalization
    meta_data_file = os.path.abspath(os.path.join(root_path, "RDO", "Meta_data", f"meta_data_{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossless.json"))
    meta_data = load_metadata(meta_data_file)
    if meta_data is None:
        print("Error: Metadata file could not be loaded.")
        return

    # Find template PLY file from the largest iteration
    ...
    for pq_rest, pq_dc, pq_opacity in allowed_pq_combinations:
        ...
        assign_attributes_and_save(template_ply_file, output_file, denormalized_dc, denormalized_rest, denormalized_opacity, denormalized_scale, denormalized_rot)
