...
def process_ply_files(base_path):
    ...
    # Define the paths
    reconstructed_path = os.path.join(base_path, 'reconstructed_3DGS',  f"{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossless")
    test_model_path = os.path.join(base_path, 'test_model')
    rdo_psnr_path = os.path.join(base_path, 'RDO', 'PSNR', f"{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossless")
    rdo_psnr_per_view_path = os.path.join(base_path, 'RDO', 'PSNR_per_view', f"{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossless")
    attributes_compressed_path = os.path.join(base_path, 'attributes_compressed')
    bpp_path = os.path.join(base_path, 'RDO', 'bpp', f"{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossless")
    ...
    for ply_file in ply_files:
        ...
        attributes_folder = os.path.join(attributes_compressed_path, f"{dataset_name}_depth_{depth}_thr_{thr}_{mode}_{suffix}_lossless")

        # Process decompression logs
        for subfolder in ['dc_decompressed', 'rest_decompressed', 'opacity_decompressed', 'scale_decompressed', 'rot_decompressed']:
    ...
    for ply_file in ply_files:
        ...
        render_command = [
            'python', 'render.py', '-m', dataset_model_folder,
        ...
        metrics_command = [
            'python', 'metrics.py', '-m', dataset_model_folder
        ]
